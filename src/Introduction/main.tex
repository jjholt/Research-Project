...
For soft tissue low-frequency stimulations, patients are able to distinguish between 1 Hz and 2 Hz with an accuracy of 71\% \cite{ChaubeyPravin2014Cvhf}, while in the range of \SIrange{60}{360}{Hz} patients have been shown to be able to distinguish 6 levels of vibration \cite{rosenbaum-chou_development_2016}.
It is unclear whether osseoperception allows for similar resolution, but there are some studies suggesting the use of frequencies in the range of \SIrange{1}{250}{Hz} \cite{jacobs_evaluation_2000}.
...
It uses a modal analysis to help inform the appropriate frequency range, then applies a concentrated point-force on the spigot prescribing a linear displacement according to equation \ref{eq:sin-eq}, where $\textbf{s}$ is displacement, $ A $ is amplitude, $t$ is time, $f$ is linear frequency, $z$ is a Cartesian axis.
...
\paragraph{Variables}
We explore the effects of each of the following independent variables as they pertain to the periodic force input: frequency, amplitude, duration and position.
...
A \SI{6}{mm} high, \SI{14}{mm} radius collar sits on the distal bone cortex.
An external spigot has a height of \SI{45}{mm} and radius of \SI{9}{mm}.

The CAD model has a hexahedral-dominated \SI{2.5}{mm} mesh (element types C3D8R and C3D6) with 4300 elements and properties that of Titanium 6--4, i.e., elastic material of density \SI{4430}{\kilo\gram\per\metre\cubed}, Young's modulus \SI{114}{\giga\pascal} and Poisson's ratio 0.33.
