...
maxima = zeros(1,length(frequencies));
% collected_data = collected_data(2:end);
for i = 1:numel(collected_data)
    datum = collected_data{i};
    magnitude = sqrt(sum(datum(1:end, 2:4).^2, 2));
    maxima(i) = max(magnitude);
end
scatter(frequencies, maxima)
set(gca, 'yscale', 'log')
xlabel("Frequency [Hz]"); ylabel("Maximum displacement [m]")
title("Maximum total displacement")
output = horzcat(frequencies', maxima');
writematrix(output, "maxima.csv")